$(document).ready(function () {
    ...
    // --- 1. Kích hoạt Hero Banner Slider ---
    if ($("#hero-slider").length) { // Kiểm tra xem trang có slider không mới chạy
    ...
    // --- 2. LOGIC SMART NAVBAR ẨN/HIỆN KHI CUỘN TRANG ---
    var lastScrollTop = 0;
    ...
    // --- 3. XỬ LÝ SEARCH OVERLAY ---
    // Khi bấm vào icon kính lúp trên Navbar
    $("#search-trigger").click(function (e) {
        e.preventDefault(); // Chặn chuyển trang
        $("#search-overlay").addClass("active"); // Hiện overlay
        setTimeout(function () {
            $(".search-input-transparent").focus(); // Đợi overlay hiện ra rồi trỏ chuột vào
        }, 100);
    });
    ...
    // Khi bấm phím ESC trên bàn phím thì cũng đóng luôn
    ...
    // --- 5. Kích hoạt Bootstrap Tooltips ---
    ...
    // =========================================================
    // CÁC CHỨC NĂNG DÀNH RIÊNG CHO TRANG "ĐĂNG CÔNG THỨC"
    // =========================================================

    // --- 6. Thêm/Xóa nguyên liệu động ---
    ...
    // --- 7. Thêm/Xóa bước làm động ---
    $('#add-step').click(function () {
        var stepCount = $('.step-item').length + 1;
        var html = `
            <div class="step-item mb-4">
                <div class="d-flex justify-content-between mb-2">
                    <label class="fw-bold text-orange step-label">Bước ${stepCount}</label>
    ...
    // Xóa bước làm và Cập nhật lại số thứ tự
    $(document).on('click', '.btn-remove-step', function () {
        $(this).closest('.step-item').remove();

        // Cập nhật lại chữ "Bước 1, Bước 2" cho đúng thứ tự sau khi xóa
        $('.step-item').each(function (index) {
            $(this).find('.step-label').text('Bước ' + (index + 1));
        });
    });
...
// =========================================================
// HÀM CHẠY ĐỘC LẬP (KHÔNG DÙNG JQUERY)
// =========================================================

// --- 8. Chuyển đổi đơn vị nguyên liệu ---
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Chỉ chạy chức năng này nếu trên trang có tồn tại cái bảng tính đổi đơn vị
    if (inputVal && unitSelect && outputVal) {
        function convert() {
